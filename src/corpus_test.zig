...
fn getCorpusConfig(alloc: std.mem.Allocator) ?[]const u8 {
    const root_c = std.c.getenv("PHPCMA_CORPUS_ROOT") orelse return null;
...
fn corpusAvailable() bool {
    ...
    std.Io.Dir.accessAbsolute(types.io, config_path, .{}) catch return false;
    return true;
}
...
test "corpus: check-types produces consistent violation count" {
    ...
    // Violation count within expected range, allow ±10%.
    //
    // Baseline 121: pinned after the type/visibility/interface analyzer was
    // hardened to eliminate false positives that previously inflated the count
    // (first-class-callable `f(...)` mis-counted as a 0-arg call; protected
    // `parent::` calls and same-class private calls; covariant return types;
    // implementations adding optional parameters; abstract classes "missing"
    // interface methods; `use X as Y` alias mis-resolution). The remaining
    // violations are all genuine cross-project return-type boundary warnings.
    const expected: usize = 121;
    const lower = expected - expected / 10; // 109
    const upper = expected + expected / 10; // 133
