...
fn symbolCollectWorker(ctx: *const SymbolCollectContext, thread_idx: usize, chunk_start: usize, chunk_end: usize) void {
    ...
    while (i < chunk_end) : (i += 1) {
        ...
        const source = std.Io.Dir.cwd().readFileAlloc(types.io, file_path, result.allocator, .limited(max_file_size)) catch continue;
...
fn sequentialSymbolCollect(
    ...
    for (files) |file_path| {
        const source = std.Io.Dir.cwd().readFileAlloc(types.io, file_path, allocator, .limited(max_file_size)) catch continue;
        const tree = parser.parseString(source, null) orelse continue;
        defer tree.destroy();
...
fn callAnalysisWorker(ctx: *const CallAnalysisContext, thread_idx: usize, chunk_start: usize, chunk_end: usize) void {
    ...
    while (i < chunk_end) : (i += 1) {
        ...
        var analyzer = CallAnalyzer.init(result.allocator, ctx.sym_table, file_ctx_ptr);
        ...
        for (analyzer.getCalls()) |call| {
            ...
            var copied_arg_types = false;
            if (call.arg_types.len > 0) {
                owned_call.arg_types = ctx.allocator.dupe(?types.TypeInfo, call.arg_types) catch {
                    ctx.allocator.free(owned_call.caller_fqn);
                    ctx.allocator.free(owned_call.callee_name);
                ...
                owned_call.owns_arg_types = true;
                copied_arg_types = true;
            }
            ...
            result.calls.append(result.allocator, owned_call) catch {
                ...
                if (copied_arg_types) {
                    ctx.allocator.free(owned_call.arg_types);
                }
                continue;
            };
        }
...
fn createTempPhpFile(allocator: std.mem.Allocator, dir: std.Io.Dir, name: []const u8, content: []const u8) ![]const u8 {
    const file = try dir.createFile(types.io, name, .{});
    defer file.close(types.io);
    try file.writeStreamingAll(types.io, content);
    var buf: [std.fs.max_path_bytes]u8 = undefined;
    const n = try dir.realPathFile(types.io, name, &buf);
...
test "parallel: call analysis matches sequential" {
    ...
    for (file_paths[0..created]) |fp| {
        const src = try std.Io.Dir.cwd().readFileAlloc(types.io, fp, allocator, .limited(max_file_size));
    ...
    // Sequential call analysis. The single-threaded analyzer allocates
    // arena-designed FQN strings directly, so give it a local arena (the
    // parallel path below dups into the thread-safe caller allocator and frees
    // via ProjectCallGraph.deinit instead).
    var seq_arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer seq_arena.deinit();
    const seq_alloc = seq_arena.allocator();
    var seq_graph = ProjectCallGraph.init(seq_alloc, &sym_table);
    defer seq_graph.deinit();
    sequentialCallAnalysis(seq_alloc, &file_paths, &file_sources, &file_contexts, &sym_table, &seq_graph);
...
test "safety: 100+ files → call graph parity with sequential" {
    ...
    // Run sequential for comparison. The single-threaded path allocates
    // arena-designed strings directly, so use a local arena (the parallel
    // pipeline above frees its thread-safe-allocator dups via deinit).
    var seq_arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer seq_arena.deinit();
    const seq_alloc = seq_arena.allocator();
    var configs = [_]ProjectConfig{};
    var seq_sym = SymbolTable.init(seq_alloc);
    defer seq_sym.deinit();
    var seq_ctx = std.StringHashMap(FileContext).init(seq_alloc);
    defer {
        var it = seq_ctx.valueIterator();
        while (it.next()) |v| @constCast(v).deinit();
        seq_ctx.deinit();
    }
    var seq_src = std.StringHashMap([]const u8).init(seq_alloc);
    defer {
        var it = seq_src.valueIterator();
        while (it.next()) |v| seq_alloc.free(v.*);
        seq_src.deinit();
    }
    sequentialSymbolCollect(seq_alloc, file_paths[0..created], &configs, &seq_sym, &seq_ctx, &seq_src, testCollectFn);
    try seq_sym.resolveInheritance();

    var seq_graph = ProjectCallGraph.init(seq_alloc, &seq_sym);
    defer seq_graph.deinit();
    sequentialCallAnalysis(seq_alloc, file_paths[0..created], &seq_src, &seq_ctx, &seq_sym, &seq_graph);
