...
pub const ReturnTypeChecker = struct {
    ...
    pub fn toText(self: *const ReturnTypeChecker, writer: std.Io.File) !void {
...
fn setupAndCheck(allocator: std.mem.Allocator, source: []const u8) !TestCtx {
    ...
    var collector = main_mod.SymbolCollector.init(allocator, &ctx.sym_table, &ctx.file_ctx, source);
