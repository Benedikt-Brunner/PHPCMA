...
/// Monotonically-unique suffix for temp file names (replaces the removed
/// `std.time.milliTimestamp`).
fn uniqueSuffix() u64 {
    return @intCast(std.Io.Clock.now(.real, types.io).nanoseconds);
}
...
fn writeTempPhp(allocator: std.mem.Allocator, php_source: []const u8) ![]const u8 {
    ...
    const file = try std.Io.Dir.createFileAbsolute(types.io, path, .{});
    defer file.close(types.io);
    try file.writeStreamingAll(types.io, php_source);

    return path;
}

fn cleanupTempFile(path: []const u8) void {
    std.Io.Dir.deleteFileAbsolute(types.io, path) catch {};
}

fn phpAvailable() bool {
    const result = std.process.run(std.testing.allocator, types.io, .{
        .argv = &.{ "php", "--version" },
    }) catch return false;
...
fn runReflect(allocator: std.mem.Allocator, file_path: []const u8) !ReflectedOutput {
    const result = try std.process.run(allocator, types.io, .{
    ...
    if (result.term != .exited or result.term.exited != 0) {
        std.debug.print("reflect.php failed:\n{s}\n", .{result.stderr});
...
fn runMultiFileDifferentialTest(files: []const struct { name: []const u8, source: []const u8 }) !void {
    ...
    for (files) |f| {
        ...
        const file = try std.Io.Dir.createFileAbsolute(types.io, path, .{});
        defer file.close(types.io);
        try file.writeStreamingAll(types.io, f.source);
    ...
    for (files, 0..) |f, i| {
        ...
        var collector = SymbolCollector.init(arena_alloc, &sym_table, &file_ctx, f.source);
        try collector.collect(tree);
    }
    ...
    const result = try std.process.run(arena_alloc, types.io, .{
        .argv = argv.items,
        .cwd = .{ .path = getProjectRoot() },
    });
...
fn phpVersionId(allocator: std.mem.Allocator) !u32 {
    ...
    if (result.term != .exited or result.term.exited != 0) return error.PhpUnavailable;
