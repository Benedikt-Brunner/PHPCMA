...
pub fn runPipelineOnFuzzInput(alloc: std.mem.Allocator, source: []const u8, tree: *ts.Tree) !void {
    ...
    // Call analysis
    var analyzer = CallAnalyzer.init(alloc, &sym_table, &file_ctx);
...
fn writeTempPhpFileAbsolute(alloc: std.mem.Allocator, dir: std.Io.Dir, file_name: []const u8, source: []const u8) ![]const u8 {
    const file = try dir.createFile(types.io, file_name, .{});
    defer file.close(types.io);
    try file.writeStreamingAll(types.io, source);

    var path_buf: [std.fs.max_path_bytes]u8 = undefined;
    const n = try dir.realPathFile(types.io, file_name, &path_buf);
    return try alloc.dupe(u8, path_buf[0..n]);
}
...
/// Zig 0.16 passes a `*std.testing.Smith` to fuzz callbacks instead of a raw
/// byte slice. Fill a stack buffer from the Smith and hand the existing
/// byte-slice-based pipelines their input. The caller owns `buf`, which must
...
fn fuzzPhpPipeline(_: void, smith: *std.testing.Smith) anyerror!void {
...
fn fuzzFullPipelineNeverCrashes(_: void, smith: *std.testing.Smith) anyerror!void {
    var input_buf: [4096]u8 = undefined;
    const input = fuzzInput(smith, &input_buf);
...
fn fuzzParallelPipelineNeverCrashes(_: void, smith: *std.testing.Smith) anyerror!void {
    var input_buf: [4096]u8 = undefined;
    const input = fuzzInput(smith, &input_buf);
    ...
    const root_len = tmp.dir.realPathFile(types.io, ".", &root_buf) catch return;
    const root_path_owned = alloc.dupe(u8, root_buf[0..root_len]) catch return;
...
fn fuzzSymbolCollector(_: void, smith: *std.testing.Smith) anyerror!void {
    var input_buf: [4096]u8 = undefined;
    const input = fuzzInput(smith, &input_buf);
...
fn fuzzPhpDocParser(_: void, smith: *std.testing.Smith) anyerror!void {
    var input_buf: [4096]u8 = undefined;
    const input = fuzzInput(smith, &input_buf);
...
fn fuzzConfigParser(_: void, smith: *std.testing.Smith) anyerror!void {
    var input_buf: [4096]u8 = undefined;
    const input = fuzzInput(smith, &input_buf);
