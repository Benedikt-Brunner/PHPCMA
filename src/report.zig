...
pub const UnifiedReport = struct {
        ...
        for (call_graph.calls.items) |call| {
            ...
            if (call.resolved_target != null) {
                switch (call.resolution_method) {
                    ...
                    .interface_single_impl, .di_config_binding, .interface_contract => self.type_checks.interface_compliance.pass += 1,
                    .unresolved => {},
                }
            } else {
    ...
    pub fn toText(self: *const UnifiedReport, file: std.Io.File) !void {
    ...
    pub fn toJson(self: *const UnifiedReport, file: std.Io.File) !void {
        var buf: [4096]u8 = undefined;
        var w = file.writer(types.io, &buf);
        try self.writeJson(&w.interface);
        try w.interface.flush();
    }

    /// Render the report as JSON to any `std.Io.Writer`. The file-based `toJson`
    /// delegates here; the MCP `report` tool drives this with an allocating
    /// writer so the served payload is byte-identical to the CLI's JSON output.
    pub fn writeJson(self: *const UnifiedReport, writer: *std.Io.Writer) !void {
        try writer.writeAll("{\n");
    ...
    pub fn toSarif(self: *const UnifiedReport, file: std.Io.File) !void {
        var buf: [4096]u8 = undefined;
        var w = file.writer(types.io, &buf);
        const writer = &w.interface;
    ...
    pub fn toCheckstyle(self: *const UnifiedReport, file: std.Io.File) !void {
        var buf: [4096]u8 = undefined;
        var w = file.writer(types.io, &buf);
        const writer = &w.interface;
...
/// Capture the output of one of `UnifiedReport`'s `to*` emitters by writing to
/// a temp file and reading it back. Replaces the Zig 0.15 `std.posix.pipe`
/// capture pattern, which is unavailable under the 0.16 `std.Io` model.
/// Caller owns the returned slice.
fn captureReport(
    alloc: std.mem.Allocator,
    rpt: *const UnifiedReport,
    comptime emit: []const u8,
) ![]u8 {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    const f = try tmp.dir.createFile(types.io, "out", .{});
    try @field(UnifiedReport, emit)(rpt, f);
    f.close(types.io);
    return tmp.dir.readFileAlloc(types.io, "out", alloc, .limited(1 << 20));
}
...
fn buildTestGraph(allocator: std.mem.Allocator, source: []const u8) !struct { *SymbolTable, *ProjectCallGraph } {
    ...
    var analyzer = call_analyzer.CallAnalyzer.init(allocator, sym_table, file_ctx);
...
test "UnifiedReport: report with multiple phases" {
    ...
    const dev_null = std.Io.Dir.openFileAbsolute(types.io, "/dev/null", .{ .mode = .write_only }) catch return;
    defer dev_null.close(types.io);
...
test "UnifiedReport: SARIF valid JSON with schema and version" {
    ...
    const out_file = try tmp.dir.createFile(types.io, "sarif.json", .{});
    try unified_report.toSarif(out_file);
    out_file.close(types.io);

    const sarif_content = try tmp.dir.readFileAlloc(types.io, "sarif.json", alloc, .limited(64 * 1024));
...
test "UnifiedReport: Checkstyle valid XML structure" {
    ...
    const output = try captureReport(alloc, &unified_report, "toCheckstyle");
...
test "null safety regression: JSON output includes null-safety violations" {
    ...
    const output = try captureReport(alloc, &report, "toJson");
...
test "null safety regression: text output shows null safety row" {
    ...
    const output = try captureReport(alloc, &report, "toText");
...
test "dead code section: JSON output includes dead_code counts and candidates" {
    ...
    const output = try captureReport(alloc, &r, "toJson");
...
test "dead code section: text output includes dead code header" {
    ...
    const output = try captureReport(alloc, &r, "toText");
