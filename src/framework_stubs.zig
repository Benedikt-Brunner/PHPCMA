...
/// Sentinel `file_path` for every synthesized framework-stub symbol. Stubs carry
/// only the members PHPCMA needs to resolve calls, *not* the real vendor
/// ancestry, so downstream analyzers must treat a stub's inheritance as
/// incomplete (unknown) rather than authoritative.
pub const STUB_FILE_PATH = "<framework-stub>";
...
fn stubMethod(
    ...
    comptime params: []const ParameterInfo,
...
fn addClass(allocator: std.mem.Allocator, sym_table: *SymbolTable, fqcn: []const u8, methods: []const MethodSymbol, extends: ?[]const u8) !void {
    ...
    class.file_path = STUB_FILE_PATH;
...
fn addInterface(allocator: std.mem.Allocator, sym_table: *SymbolTable, fqcn: []const u8, methods: []const MethodSymbol) !void {
    ...
    iface.file_path = STUB_FILE_PATH;
