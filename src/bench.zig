...
fn createTempFile(allocator: std.mem.Allocator, dir: std.Io.Dir, name: []const u8, content: []const u8) ![]const u8 {
    dir.writeFile(types.io, .{ .sub_path = name, .data = content }) catch |err| {
        std.debug.print("Failed to write temp file {s}: {}\n", .{ name, err });
        return err;
    };
    const real = try dir.realPathFileAlloc(types.io, name, allocator);
    return real;
}

/// Generate N PHP fixture files in a temp directory, return file paths.
fn generateFixtureFiles(allocator: std.mem.Allocator, dir: std.Io.Dir, count: usize) ![]const []const u8 {
...
test "bench: pass 2 isolation < 40% of total" {
    ...
    var total_timer = types.Timer.start();
    {
        var sym_table = SymbolTable.init(allocator);
        defer sym_table.deinit();
        var file_contexts = std.StringHashMap(FileContext).init(allocator);
    ...
    // Measure Pass 2 only
    var pass2_timer = types.Timer.start();
    {
        var sym_table = SymbolTable.init(allocator);
        defer sym_table.deinit();
        var file_contexts = std.StringHashMap(FileContext).init(allocator);
...
test "bench: pass 4 isolation < 70% of total" {
    ...
    var pass4_timer = types.Timer.start();
    {
...
test "bench: parallel vs sequential comparison for 500 files" {
    ...
    var seq_timer = types.Timer.start();
    {
        // The analysis symbols/calls are arena-designed (their deinit frees
        // structural state, not per-symbol strings). The single-threaded
        // sequential path here allocates those strings directly, so give it a
        // local arena that mirrors the production top-level arena.
        var seq_arena = std.heap.ArenaAllocator.init(std.testing.allocator);
        defer seq_arena.deinit();
        const seq_allocator = seq_arena.allocator();

        var sym_table = SymbolTable.init(seq_allocator);
        defer sym_table.deinit();
        var file_contexts = std.StringHashMap(FileContext).init(seq_allocator);
        defer {
            var it = file_contexts.valueIterator();
            while (it.next()) |v| @constCast(v).deinit();
            file_contexts.deinit();
        }
        var file_sources = std.StringHashMap([]const u8).init(seq_allocator);
        defer {
            var it = file_sources.valueIterator();
            while (it.next()) |v| seq_allocator.free(v.*);
            file_sources.deinit();
        }
        ...
        for (paths) |file_path| {
            const source = std.Io.Dir.cwd().readFileAlloc(types.io, file_path, seq_allocator, .limited(max_file_size)) catch continue;
            const tree = parser.parseString(source, null) orelse continue;
            defer tree.destroy();

            var file_ctx = FileContext.init(seq_allocator, file_path);
            main_mod.collectSymbolsFromSource(seq_allocator, &sym_table, &file_ctx, source, php_lang, tree) catch continue;
        ...
        var call_graph = ProjectCallGraph.init(seq_allocator, &sym_table);
        defer call_graph.deinit();
        ...
        for (paths) |file_path| {
            ...
            var analyzer = CallAnalyzer.init(seq_allocator, &sym_table, file_ctx_ptr);
    ...
    // Parallel run
    var par_timer = types.Timer.start();
    {
        var sym_table = SymbolTable.init(allocator);
        defer sym_table.deinit();
        var file_contexts = std.StringHashMap(FileContext).init(allocator);
...
test "bench: file caching no double-read in pass 4" {
    ...
    for (paths) |p| {
        ...
        tmp_dir.dir.deleteFile(types.io, basename) catch {};
    }
